...
namespace CGSpecFlowBVT.Page_Objects
{
    public static class ProblemPage
    {
        ...
        private static bool IsWindowTitleTicketNumberVisible(string ticketNum)
        {
            ...
            {
                log.Info("Problem form window title ticket number was not found.");
            }
            else
            {
                log.Info("Problem form window title ticket number was found.");
            }
        ...
        private static bool IsFormTicketNumberVisible(string ticketNum)
        {
            ...
            isTicketNumberVisible &= GeneralPage.ExtractTextFromElement(ID.ticketInfo).Contains("PR");

            if (!(isTicketNumberVisible))
            {
                log.Info("Problem form ticket details ticket number was not found.");
            }
            else
            {
                log.Info("Problem form ticket details ticket number was found.");
            }
            return isTicketNumberVisible;
        }
